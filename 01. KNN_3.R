rm(list = is())
setwd("C:/Users/vicky/OneDrive/??????/Coding and Data Literacy/Kaggle/CarEvaluationData")
...
mean_values <- 0
set.seed(1988)
# how to determine k value? The proper starting is the sqrt the numeber of the observations.
k_estimate <- round(sqrt(nrow(data0)), 0)
explanatory_data <- tibble(k_value = 1:(k_estimate + 8))
for(i in 1:50){
...
mean_values <- mean_values %>%
  rbind(k_value = c(1:(k_estimate + 8)))
...
ggplot(mean_values_convert, aes(x = k_value, y = mean_values)) +
  geom_point() +
  geom_line() +
  scale_x_continuous(name = "K_value",breaks=seq(0,k_estimate + 8,5)) +# to set the x-axis. seq() means from 0 to 10 and by = 2.
  labs(y = "Accuracy",
       title = "Accuracy vs. K values by KNN Model")

k_selected <- which.max(mean_values[1,]) # 

# Therefore, for this case, k = 6
mdl_selected <- knn(train = train[,-1], test = test[,-1], cl = train_decision, k = k_selected)
data2 <- data1 %>%
